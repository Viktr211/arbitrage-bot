...
st.markdown('<h1 class="main-header">🚀 НАКОПИТЕЛЬНЫЙ АРБИТРАЖ PRO v7.7</h1>', unsafe_allow_html=True)
...
for key, default in {
    ...
    'history': [],
    'portfolio': {asset: random.uniform(0.1, 10) for asset in DEFAULT_ASSETS},
    'exchanges': None
}.items():
...
with tabs[5]:
    ...
    if st.button("Запросить вывод"):
        st.success("Заявка на вывод отправлена (в реальной версии будет обрабатываться по вторникам и пятницам)")
...
st.caption("Накопительный Арбитраж PRO v7.7")
