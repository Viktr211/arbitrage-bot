...
st.set_page_config(page_title="Арбитражный бот | Ручные балансы", layout="wide", page_icon="🔄", initial_sidebar_state="collapsed")
...
# Параметры арбитража (можно менять в админке позже)
MIN_SPREAD_PERCENT = 0.001
FEE_PERCENT = 0.01
SLIPPAGE_PERCENT = 0.01
TRADE_PERCENT = 50          # 50% от доступных USDT/токенов
MIN_TRADE_USDT = 5.0
SCAN_INTERVAL = 5
...
# ---------- БАЛАНСЫ ПОЛЬЗОВАТЕЛЯ (доступ через st.session_state) ----------
...
def update_balance(exchange_name, asset, delta):
    if asset == 'USDT':
        st.session_state.user_balances[exchange_name]['USDT'] = st.session_state.user_balances[exchange_name].get('USDT', 0.0) + delta
    else:
...
if not st.session_state.logged_in:
    st.markdown('<h1 class="main-header">🔄 Арбитражный бот | Ручные балансы</h1>', unsafe_allow_html=True)
...
# TAB 5: Балансы (ручное управление)
with tabs[5]:
    st.subheader("💼 Управление балансами по биржам")
    st.info("Здесь вы можете вручную добавлять USDT и токены на каждую биржу. После добавления нажмите 'Обновить данные' выше.")
    for ex in EXCHANGES:
        with st.expander(f"### {ex.upper()}"):
            ...
            # Добавление USDT
            add_usdt = st.number_input(f"Добавить USDT на {ex.upper()}", min_value=0.0, step=100.0, key=f"add_usdt_{ex}")
            if st.button(f"➕ Добавить {add_usdt} USDT", key=f"btn_usdt_{ex}"):
                if add_usdt > 0:
                    update_balance(ex, 'USDT', add_usdt)
                    st.success(f"Добавлено {add_usdt} USDT на биржу {ex.upper()}")
                    st.rerun()
            # Добавление токенов
            st.write("**Добавить токены:**")
            for asset in get_available_tokens():
                col1, col2 = st.columns([3,1])
                with col1:
                    amount = st.number_input(f"{asset}", min_value=0.0, step=0.01, key=f"add_{ex}_{asset}", format="%.6f")
                with col2:
                    if st.button(f"➕ {asset}", key=f"btn_{ex}_{asset}"):
                        if amount > 0:
        ...
        with a5:
            ...
            for w in pend:
                ...
                if st.button(f"Выполнить", key=f"comp_{w['id']}"):
                    ...
                    st.rerun()
        with a6:
            st.write("Настройки арбитража (пока в коде, позже можно вынести в интерфейс)")
