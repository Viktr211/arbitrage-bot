...
def real_sell_with_liquidity(exchange, token, amount_token, max_slippage=0.3):
    if not exchange: return False, "Биржа не подключена", None
    price, available = get_order_book_price(exchange, token, 'sell', amount_token * 100)
    if price is None:
        try:
            ticker = exchange.fetch_ticker(f"{token}/USDT")
...
def execute_real_arbitrage(opp, user_id):
        ...
        if not can_trade_real(st.session_state.email):
            ...
            if last is None or (now - last).total_seconds() >= interval:
                ...
                opp = find_real_opportunity(
                    ...
                    max_slippage=0.3, real_exchanges=st.session_state.real_exchanges
                )
                if opp:
        ...
        with st.expander(f"{ex.upper()}"):
            ...
            with colA:
                ...
                if st.button(f"Купить {token_buy}", key=f"btn_buy_{ex}"):
                    if st.session_state.trade_mode == "Реальный":
                        if st.session_state.real_exchanges.get(ex):
                            ok, msg, _ = real_buy_with_liquidity(st.session_state.real_exchanges[ex], token_buy, usdt_amt, max_slippage=0.3)
                        else:
            ...
            with colB:
                ...
                if st.button(f"Продать {token_sell}", key=f"btn_sell_{ex}"):
                    if st.session_state.trade_mode == "Реальный":
                        if st.session_state.real_exchanges.get(ex):
                            ok, msg, _ = real_sell_with_liquidity(st.session_state.real_exchanges[ex], token_sell, token_amt, max_slippage=0.3)
                        else:
                            ok, msg = False, "Биржа не подключена"
                    else:
                        if not st.session_state.demo_data:
