...
with tabs[2]:
    ...
    if opps:
        ...
        for opp in opps[:10]:
            st.info(f"🎯 {opp['asset']}: купить на {opp['buy_exchange'].upper()} ${opp['buy_price']:.2f} → продать на {opp['sell_exchange'].upper()} ${opp['sell_price']:.2f} | чистая прибыль: {opp['net_profit']:.2f} USDT")
    else:
