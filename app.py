...
# ------------------- БАЗА ДАННЫХ (функции без кэша для записи) -------------------
...
def get_cached_price(exchange_name, symbol):
    clients = init_public_clients()
    exch = clients.get(exchange_name)
    if exch:
        try:
            ticker = exch.fetch_ticker(f"{symbol}/USDT")
            return ticker['last']
        except:
            return None
    return None
