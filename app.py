...
st.set_page_config(
    page_title="Накопительный арбитражный бот",
    layout="wide",
    page_icon="🔄",
...
DEFAULT_ASSETS = [
    "BTC", "ETH", "SOL", "BNB", "XRP",
    "ADA", "AVAX", "LINK", "SUI", "HYPE", "TON"
]
DEFAULT_PORTFOLIO = {
    "BTC": 0.08, "ETH": 1.5, "SOL": 25.0, "BNB": 4.0, "XRP": 1800.0,
    "ADA": 4000.0, "AVAX": 40.0, "LINK": 70.0, "SUI": 400.0,
    "HYPE": 50.0, "TON": 80.0
}
...
def create_user(email, pwd_hash, full_name, country, city, phone, wallet):
    initial_balances = {
        ex: {"USDT": DEMO_USDT_PER_EXCHANGE, "portfolio": DEFAULT_PORTFOLIO.copy()}
        for ex in EXCHANGES
    }
    data = {
        "email": email,
    ...
    if status == "completed":
        ...
        if w.data:
            ...
            user = (
                ...
                .select("withdrawable_balance, total_admin_fee_paid")
                .eq("id", uid)
                .execute()
            )
            if user.data:
                ...
                supabase.table("users").update(
                    {"withdrawable_balance": new_bal, "total_admin_fee_paid": new_fee}
                ).eq("id", uid).execute()
        ...
        for j, sell_ex in enumerate(EXCHANGES):
            ...
            for asset in tokens:
                ...
                if net_spread <= thresholds["min_spread_percent"]:
                    continue
                profit_before = sell_price - buy_price - (
                    buy_price * thresholds["fee_percent"] / 100
                    + sell_price * thresholds["fee_percent"] / 100
                )
                if profit_before <= 0:
                    continue
    ...
    if mode == "Демо":
        if is_admin(st.session_state.email):
            ...
            if sell_ex in st.session_state.user_balances:
                ...
                st.session_state.user_balances[sell_ex] = {
                    "USDT": DEMO_USDT_PER_EXCHANGE + profit,
            ...
            if sell_ex in st.session_state.user_balances:
                ...
                st.session_state.user_balances[sell_ex] = {
                    "USDT": DEMO_USDT_PER_EXCHANGE + reinvest,
                    "portfolio": DEFAULT_PORTFOLIO.copy(),
                }
...
# ---------- ФОНОВОЙ ПОТОК ДЛЯ АВТО-СДЕЛОК ----------
def start_auto_trade():
    """Запускает фоновый поток, если он ещё не активен."""
    if "auto_trade_thread" not in st.session_state or st.session_state.auto_trade_thread is None or not st.session_state.auto_trade_thread.is_alive():
        ...
        def worker():
            while not st.session_state.get("stop_auto_trade", False):
                try:
                    with st.spinner("🔍 Поиск арбитража ..."):
                        thresholds = get_thresholds()
                        opportunities = find_all_arbitrage_opportunities(
                            st.session_state.exchanges, thresholds
                        )
                        if opportunities:
...
with tabs[3]:
    ...
    if token_stats:
        data = []
        for t, d in sorted(token_stats.items(), key=lambda x: x[1]["profit"], reverse=True):
            data.append({
                "Токен": t,
                "Сделок": d["trades"],
                "Прибыль": f"{d['profit']:.2f}",
                "% общ.": f"{d['profit'] / total_profit_all * 100:.1f}%",
            })
        st.dataframe(pd.DataFrame(data), use_container_width=True, hide_index=True)
        fig = px.pie(
            pd.DataFrame([{"Токен": t, "Прибыль": d["profit"]} for t, d in token_stats.items()]),
            values="Прибыль",
            names="Токен",
            title="Доля прибыли",
        )
...
if show_admin:
    with tabs[-1]:
        ...
        with a1:
            ...
            if users:
                df = pd.DataFrame([{
                    "Email": u["email"],
                    "Имя": u["full_name"],
                    "Статус": u["registration_status"],
                    "Прибыль": f"${u.get('total_profit', 0):.2f}",
        ...
        with a4:
            ...
            for ex in EXCHANGES:
                with st.expander(f"🔑 {ex.upper()}"):
                    ...
                    new_api = st.text_input("API Key", value=cur_api, type="password", key=f"api_{ex}")
                    new_sec = st.text_input("Secret Key", value=cur_sec, type="password", key=f"sec_{ex}")
        ...
        with a5:
            ...
            if trades:
                df = pd.DataFrame([{
                    "Пользователь": t.get("users", {}).get("email", ""),
                    "Токен": t["asset"],
                    "Прибыль": f"${t['profit']:.2f}",
                    "Время": t["trade_time"],
                } for t in trades])
                st.dataframe(df, use_container_width=True, hide_index=True)
        with a6:
            ...
            for w in pend:
                ...
                if st.button("Выполнить", key=f"comp_{w['id']}"):
