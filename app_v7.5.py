...
st.set_page_config(page_title="Накопительный Арбитраж PRO v7.5", layout="wide", page_icon="🚀")
...
st.markdown('<h1 class="main-header">🚀 НАКОПИТЕЛЬНЫЙ АРБИТРАЖ PRO v7.5</h1>', unsafe_allow_html=True)
...
for key, default in {
    ...
    'trade_count': 0,
    'history': [],
    'portfolio': {asset: random.uniform(0.01, 5) for asset in DEFAULT_ASSETS},
...
def find_all_arbitrage_opportunities():
    ...
    for asset in DEFAULT_ASSETS:
        ...
        if price:
            main_price_dict[asset] = price

    for asset in DEFAULT_ASSETS:
        if asset not in main_price_dict:
            continue
        main_price = main_price_dict[asset]
        for aux in AUX_EXCHANGES:
            if aux in st.session_state.exchanges:
                ...
                if aux_price and aux_price < main_price:
                    ...
                    profit = round((main_price - aux_price) * 0.8, 2)
...
st.caption("Накопительный Арбитраж PRO v7.5")
